...
import {
  ...
  Image as ImageIcon, Eye, X, Search, TrendingUp,
...
const FilterPill = ({ label, active, count, onClick }: {
  ...
  <button
    onClick={onClick}
    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-body border transition-all duration-150 ${
      active
        ? "bg-pink-soft text-primary border-primary/30 font-semibold"
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-body font-semibold border ${m.cls}`}>
      {m.label}
    </span>
  );
};
...
const AdminDashboard = () => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(o => {
      ...
      const matchSearch = !q
        || o.mood?.toLowerCase().includes(q)
        || o.recipient_type?.toLowerCase().includes(q)
        || (o.user_description ?? "").toLowerCase().includes(q);
      ...
      <div className="bg-card border-b border-border/60 px-6 flex gap-1">
        {(["orders", "flowers"] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab}
            {tab === "orders" && (
              <span className="text-[10px] bg-pink-soft text-primary px-1.5 py-0.5 rounded-full">{orders.length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="px-6 py-6 max-w-6xl mx-auto">
        ...
        {activeTab === "orders" && (
          <div>
                ...
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-6">
                    ...
                    {selectedOrder.generated_image_url
                      ...
                      : <div className="w-full md:w-44 aspect-square rounded-xl bg-pink-soft/50 flex items-center justify-center text-5xl">
                          {MOOD_EMOJI[selectedOrder.mood ?? ""] ?? "🌸"}
                        </div>
                    }
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
                        {[
                          ["Mood", selectedOrder.mood],
                          ["Recipient", selectedOrder.recipient_type],
                          ["Budget", `${selectedOrder.budget?.toLocaleString()} VNĐ`],
                          ["Date", new Date(selectedOrder.created_at).toLocaleDateString("vi-VN")],
                        ].map(([k, v]) => (
                          <div key={k}>
                      ...
                      {/* Status update */}
                      <div className="border-t border-border/40 pt-3">
                        <p className="text-[10px] font-body text-muted-foreground uppercase tracking-wider mb-2">Update status</p>
                          ...
                          {ORDER_STATUSES.map(s => {
                            ...
                            return (
                              <button key={s}
                                ...
                                className={`text-[11px] px-3 py-1.5 rounded-full font-body border transition-all ${
                                  isActive ? `${m.cls} font-semibold scale-105` : "bg-secondary text-muted-foreground border-transparent hover:opacity-80"
                                }`}>
                                {s}
                              </button>
                            );
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
              ...
              {ORDER_STATUSES.map(s => (
                <FilterPill key={s} label={STATUS_META[s].label} active={statusFilter === s}
              ...
              <div className="ml-auto flex items-center gap-2 bg-card border border-border/60 rounded-xl px-3 py-1.5">
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  placeholder="Search mood, recipient…"
                ...
                <motion.div key={order.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <div className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0">
                    {order.generated_image_url
                      ...
                      : <div className="w-full h-full bg-pink-soft/50 flex items-center justify-center text-xl">
                          {MOOD_EMOJI[order.mood ?? ""] ?? "🌸"}
                        </div>
                    }
                  </div>
        ...
        {activeTab === "flowers" && (
          <div>
              ...
              {showFlowerForm && (
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <input placeholder="Tên hoa" value={flowerForm.name}
                    ...
                    <input placeholder="Giá mỗi cành (VNĐ)" type="number" value={flowerForm.price_per_stem}
                      onChange={e => setFlowerForm(f => ({ ...f, price_per_stem: e.target.value }))}
                      className="p-3 rounded-xl border border-border/60 bg-card font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input placeholder="0 = hết hàng" type="number" min="0" value={flowerForm.stock}
                        onChange={e => setFlowerForm(f => ({ ...f, stock: e.target.value }))}
                        className="p-3 rounded-xl border border-border/60 bg-card font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
                    </div>
                    <textarea placeholder="Mô tả" value={flowerForm.description}
                    ...
                    <div className="md:col-span-2">
                      ...
                      {flowerForm.image_url && (
                        <img src={flowerForm.image_url} alt="Preview" className="w-16 h-16 object-cover rounded-lg mt-2" />
                      )}
                    </div>
                  </div>
                  <div className="flex gap-2 mt-4">
                    ...
                    <Button variant="secondary" size="sm" className="rounded-xl"
                      onClick={() => { setShowFlowerForm(false); setEditingFlower(null); }}>Huỷ</Button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
              ...
              {flowers.map(flower => (
                <motion.div key={flower.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-sm font-body text-primary font-semibold mt-2 mb-3">
                      {flower.price_per_stem.toLocaleString()} VNĐ/cành
                    </p>
                    {flower.description && (
                      <p className="text-xs text-muted-foreground font-body line-clamp-2 mb-3">{flower.description}</p>
                    )}
            ...
            {flowers.length === 0 && (
              <div className="text-center py-16 text-muted-foreground font-body text-sm">
                <Flower2 className="w-10 h-10 mx-auto mb-3 text-border" />
                Chưa có hoa nào.
              </div>
            )}
          </div>
