...
const moods: {
  ...
  { value: "romantic", label: "Romantic", icon: Heart, color: "bg-pink-soft text-pink-deep", ring: "ring-pink-deep/40", description: "Roses & soft petals", particle: "H" },
  ...
  { value: "calm", label: "Calm", icon: Leaf, color: "bg-muted text-muted-foreground", ring: "ring-muted-foreground/30", description: "Lavender & greens", particle: "o" },
...
const MoodFlowerPanel = ({
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 top-0 bottom-16 w-[260px] md:w-[300px] bg-card border border-border/60 rounded-2xl shadow-2xl shadow-black/10 z-20 flex flex-col overflow-hidden"
    >
      ...
      <p className="text-[10px] font-body text-muted-foreground px-4 py-2 border-b border-border/40 shrink-0">
        {flowers.length} flower types in this mood
      </p>
          ...
          flowers.map((flower, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] font-body text-primary">{flower.price_per_stem.toLocaleString()}d/stem</p>
              </div>

              {flower.stock === 0 ? (
                <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-red-50 text-red-600 border border-red-200 font-body shrink-0">
                  Out
                </span>
              ) : flower.stock <= 5 ? (
                <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-200 font-body shrink-0">
                  {flower.stock} left
                </span>
              ) : (
                <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-green-50 text-green-600 border border-green-200 font-body shrink-0">
                  {flower.stock} left
                </span>
              )}
            </motion.div>
...
export const MoodStep = () => {
  ...
  const [activeMood, setActiveMood] = useState<(typeof moods)[0] | null>(
    moods.find((mood) => mood.value === data.mood) ?? null
  );

  const handleSelect = (selectedMood: (typeof moods)[0]) => {
    updateData({ mood: selectedMood.value });
  ...
  return (
    <div className="flex flex-col min-h-screen px-6 md:px-12 py-8 max-w-4xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:gap-16 md:items-start flex-1">
        <motion.div
          className="md:w-64 md:shrink-0 mb-8 md:mb-0 md:sticky md:top-8"
        ...
        >
          ...
          <p className="text-muted-foreground font-body text-sm leading-relaxed mb-6">
            Choose the mood that best matches this moment. Tap a mood to preview the flowers in it, then press Next to continue.
          </p>
          <div className="flex items-center gap-2">
        ...
        <div className="flex-1 relative pb-20">
          <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {moods.map((mood) => {
              ...
              const isActive = activeMood?.value === mood.value;
              ...
              return (
                ...
                >
                  {isSelected && (
                    <motion.div
                      className="absolute inset-0 opacity-[0.12] bg-current rounded-2xl"
                      initial={{ opacity: 0 }}
                  ...
                  {!isSelected && <p className="text-[9px] font-body opacity-40 mt-auto">Tap to preview flowers</p>}
          ...
          >
            <button
              onClick={goNext}
              disabled={!data.mood}
              className="inline-flex items-center justify-center rounded-2xl px-5 py-3 text-sm font-body font-semibold transition-all bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/25 disabled:cursor-not-allowed disabled:opacity-40"
            >
              Next
            </button>
          </motion.div>
          ...
          <AnimatePresence>
            {activeMood && <MoodFlowerPanel mood={activeMood} onClose={() => setActiveMood(null)} />}
          </AnimatePresence>
        </div>
      </div>
    </div>
